...
COMMENT
ON TABLE public.patients IS
'Tabela contendo informações específicas dos pacientes.';

COMMENT
ON COLUMN public.patients.user_id IS
...
COMMENT
ON TABLE public.professionals IS
'Tabela contendo informações específicas dos profissionais.';

COMMENT
ON COLUMN public.professionals.user_id IS
'Identificador do usuário (users.id).';

COMMENT
ON COLUMN public.professionals.crp IS
'Registro profissional do psicólogo.';

COMMENT
ON COLUMN public.professionals.biography IS
'Biografia apresentada no perfil público.';

COMMENT
ON COLUMN public.professionals.therapeutic_approach IS
'Abordagem terapêutica utilizada pelo profissional.';

COMMENT
ON COLUMN public.professionals.experience_years IS
'Tempo de experiência profissional em anos.';

COMMENT
ON COLUMN public.professionals.status IS
...
COMMENT
ON TABLE public.specialties IS
'Catálogo de especialidades disponíveis para os profissionais.';

COMMENT
ON COLUMN public.specialties.id IS
'Identificador único da especialidade.';

COMMENT
ON COLUMN public.specialties.name IS
'Nome da especialidade.';

COMMENT
ON COLUMN public.specialties.description IS
'Descrição opcional da especialidade.';

COMMENT
ON COLUMN public.specialties.active IS
...
COMMENT
ON TABLE public.professional_specialties IS
'Tabela de relacionamento entre profissionais e especialidades.';

COMMENT
ON COLUMN public.professional_specialties.professional_id IS
'Profissional associado à especialidade.';

COMMENT
ON COLUMN public.professional_specialties.specialty_id IS
...
COMMENT
ON TABLE public.consultation_types IS
'Tipos de consulta oferecidos por cada profissional.';

COMMENT
ON COLUMN public.consultation_types.professional_id IS
'Profissional proprietário do tipo de consulta.';

COMMENT
ON COLUMN public.consultation_types.name IS
'Nome do tipo de consulta (Individual, Casal, Infantil, etc.).';

COMMENT
ON COLUMN public.consultation_types.description IS
'Descrição opcional do tipo de consulta.';

COMMENT
ON COLUMN public.consultation_types.duration_minutes IS
'Duração da consulta em minutos.';

COMMENT
ON COLUMN public.consultation_types.price IS
'Valor cobrado pelo profissional para este tipo de consulta.';

COMMENT
ON COLUMN public.consultation_types.active IS
...
COMMENT
ON TABLE public.professional_availability IS
'Períodos recorrentes de disponibilidade do profissional para atendimento.';

COMMENT
ON COLUMN public.professional_availability.professional_id IS
'Profissional proprietário da disponibilidade.';

COMMENT
ON COLUMN public.professional_availability.weekday IS
'Dia da semana em que a disponibilidade se repete.';

COMMENT
ON COLUMN public.professional_availability.start_time IS
'Horário inicial do período disponível.';

COMMENT
ON COLUMN public.professional_availability.end_time IS
'Horário final do período disponível.';

COMMENT
ON COLUMN public.professional_availability.active IS
...
COMMENT
ON TABLE public.absences IS
'Registra solicitações de férias e ausências temporárias dos profissionais.';

COMMENT
ON COLUMN public.absences.professional_id IS
'Profissional solicitante da ausência.';

COMMENT
ON COLUMN public.absences.type IS
'Tipo da ausência: férias ou indisponibilidade temporária.';

COMMENT
ON COLUMN public.absences.start_date IS
'Data inicial da ausência.';

COMMENT
ON COLUMN public.absences.end_date IS
'Data final da ausência.';

COMMENT
ON COLUMN public.absences.reason IS
'Justificativa informada pelo profissional.';

COMMENT
ON COLUMN public.absences.status IS
'Status da solicitação.';

COMMENT
ON COLUMN public.absences.reviewed_by IS
'Administrador responsável pela análise da solicitação.';

COMMENT
ON COLUMN public.absences.reviewed_at IS
'Data e hora da análise da solicitação.';

COMMENT
ON COLUMN public.absences.review_notes IS
...
COMMENT
ON TABLE public.appointments IS
'Agendamentos de consultas entre pacientes e profissionais.';

COMMENT
ON COLUMN public.appointments.consultation_name IS
'Nome do tipo de consulta no momento do agendamento.';

COMMENT
ON COLUMN public.appointments.consultation_duration IS
'Duração da consulta em minutos no momento do agendamento.';

COMMENT
ON COLUMN public.appointments.consultation_price IS
'Preço da consulta no momento do agendamento.';

COMMENT
ON COLUMN public.appointments.status IS
'Estado atual da consulta.';

COMMENT
ON COLUMN public.appointments.patient_confirmed IS
'Indica se o paciente confirmou presença.';

COMMENT
ON COLUMN public.appointments.professional_confirmed IS
'Indica se o profissional confirmou a consulta.';

COMMENT
ON COLUMN public.appointments.cancelled_by IS
'Quem realizou o cancelamento da consulta.';

COMMENT
ON COLUMN public.appointments.no_show_by IS
...
COMMENT
ON TABLE public.medical_records IS
'Evoluções clínicas registradas pelos profissionais após as consultas.';

COMMENT
ON COLUMN public.medical_records.appointment_id IS
'Consulta à qual esta evolução pertence.';

COMMENT
ON COLUMN public.medical_records.patient_id IS
'Paciente da evolução clínica.';

COMMENT
ON COLUMN public.medical_records.professional_id IS
'Profissional responsável pelo registro.';

COMMENT
ON COLUMN public.medical_records.evolution_number IS
'Número sequencial da evolução clínica para a consulta.';

COMMENT
ON COLUMN public.medical_records.clinical_notes IS
'Conteúdo da evolução clínica.';

COMMENT
ON COLUMN public.medical_records.is_valid IS
'Indica se a evolução está válida ou foi invalidada.';

COMMENT
ON COLUMN public.medical_records.invalidated_reason IS
...
COMMENT
ON TABLE public.conversations IS
'Representa um canal de conversa entre um paciente e um profissional.';

COMMENT
ON COLUMN public.conversations.patient_id IS
'Paciente participante da conversa.';

COMMENT
ON COLUMN public.conversations.professional_id IS
'Profissional participante da conversa.';

COMMENT
ON COLUMN public.conversations.last_message_at IS
...
COMMENT
ON TABLE public.messages IS
'Mensagens trocadas entre pacientes e profissionais.';

COMMENT
ON COLUMN public.messages.conversation_id IS
'Conversa à qual a mensagem pertence.';

COMMENT
ON COLUMN public.messages.sender_id IS
'Usuário que enviou a mensagem.';

COMMENT
ON COLUMN public.messages.receiver_id IS
'Usuário destinatário da mensagem.';

COMMENT
ON COLUMN public.messages.message_type IS
'Tipo da mensagem.';

COMMENT
ON COLUMN public.messages.content IS
'Conteúdo da mensagem.';

COMMENT
ON COLUMN public.messages.is_read IS
'Indica se a mensagem foi lida pelo destinatário.';

COMMENT
ON COLUMN public.messages.read_at IS
...
COMMENT
ON TABLE public.notifications IS
'Notificações enviadas aos usuários da plataforma.';

COMMENT
ON COLUMN public.notifications.user_id IS
'Usuário destinatário da notificação.';

COMMENT
ON COLUMN public.notifications.type IS
'Tipo da notificação.';

COMMENT
ON COLUMN public.notifications.title IS
'Título da notificação.';

COMMENT
ON COLUMN public.notifications.message IS
'Conteúdo da notificação.';

COMMENT
ON COLUMN public.notifications.reference_type IS
'Tipo do recurso relacionado à notificação.';

COMMENT
ON COLUMN public.notifications.reference_id IS
'Identificador do recurso relacionado.';

COMMENT
ON COLUMN public.notifications.read_at IS
...
COMMENT
ON TABLE public.authorized_professional_emails IS
'Lista de e-mails autorizados para cadastro de profissionais.';

COMMENT
ON COLUMN public.authorized_professional_emails.authorized_by IS
'Administrador responsável pela autorização do e-mail.';

COMMENT
ON COLUMN public.authorized_professional_emails.created_user_id IS
...
COMMENT
ON TABLE public.system_settings IS
'Configurações gerais da plataforma.';

COMMENT
ON COLUMN public.system_settings.key IS
'Nome único da configuração.';

COMMENT
ON COLUMN public.system_settings.value IS
'Valor da configuração armazenado em formato texto.';

COMMENT
ON COLUMN public.system_settings.updated_by IS
